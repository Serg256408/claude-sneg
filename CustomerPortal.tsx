...
const CustomerPortal: React.FC<CustomerPortalProps> = ({ orders, customers, onAddOrder, onUpdateOrder }) => {
  ...
  const completedOrders = useMemo(() => {
    return myOrders.filter(o => 
      normalizeOrderStatus(o.status) === OrderStatus.COMPLETED
    );
  }, [myOrders]);
...
${order.assetRequirements.map(req => {
  ...
  return `${req.type}: ${formatPrice(req.customerPrice || 0)} × ${units} = ${formatPrice((req.customerPrice || 0) * units)}`;
}).join('\n')}
...
═══════════════════════════════════════════════════════════
  ...
  const getStatusBadge = (status: OrderStatus) => {
    ...
    const styles: Record<OrderStatus, string> = {
      ...
      [OrderStatus.EXPORT_COMPLETED]: 'bg-green-600/30 text-green-400 border-green-500/40',
      [OrderStatus.REPORT_READY]: 'bg-green-600/30 text-green-400 border-green-500/40',
    };
  ...
  const getProgressSteps = (order: Order) => {
    ...
    const isFinished = normalized === OrderStatus.COMPLETED;
    ...
    return [
      ...
      { label: 'Техника в пути', done: isEnRoute || isWorking || isFinished, active: isTechAssigned && !isEnRoute && !isWorking && !isFinished, id: 3 },
      { label: 'Идёт вывоз', done: isWorking || isExporting || isFinished, active: isEnRoute, id: 4 },
                        ...
                        {currentQuote && (
                          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <button
                              ...
                              className="w-full bg-white text-slate-900 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white/90 transition-all"
                            >
                              ⬇ Скачать КП
                            </button>
                          </div>
          ...
          <div className="bg-white rounded-3xl p-6 max-w-4xl w-full shadow-2xl border border-slate-200 max-h-[90vh] overflow-y-auto text-slate-900">
                        ...
                        <div key={`${item.label}-${index}`} className="py-3 flex items-center justify-between gap-4">
                          <div>
                            ...
                            <div className="text-xs text-slate-500">
                              {item.units} {item.unitLabel} × {formatPrice(item.unitPrice)}
                            </div>
                          </div>
            ...
            <div className="flex flex-col md:flex-row gap-3 mt-6">
              <button
                ...
                className="flex-1 bg-white text-slate-900 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white/90 transition-all"
              >
                ⬇ Скачать КП
              </button>
              <button
                ...
                className="flex-1 bg-slate-900 text-white py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all"
              >
                Закрыть
              </button>
            </div>
